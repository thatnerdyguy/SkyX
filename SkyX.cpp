...
namespace SkyX
{
	SkyX::SkyX(Ogre::SceneManager* sm, Controller* c)
		...
		, mTimeOffset(0.0f)
	{
		mSkyXRoot = mSceneManager->getRootSceneNode()->createChildSceneNode();
	...
	SkyX::~SkyX()
	{
		...
		mSkyXRoot->detachAllObjects();
		mSkyXRoot->getParentSceneNode()->removeAndDestroyChild(mSkyXRoot->getName());
		mSkyXRoot = 0;
	}
